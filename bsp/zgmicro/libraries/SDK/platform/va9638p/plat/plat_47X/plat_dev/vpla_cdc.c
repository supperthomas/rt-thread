...
#define VCDC_TEST_FULL_SUPPORT (0)
...
VRET_VALUE VSYS_CDC_Init(VDRVCDCSel_t cdc_sel, VDRVCDCConfig_t *cdc_config)
{
	...
	if(g_cdc_info[VSYS_CDC_CH0IN].len == 0 && g_cdc_info[VSYS_CDC_CH0OUT].len == 0)
	{
	#if VCDC_TEST_FULL_SUPPORT
		VCDC_Init();
...
UINT32 VSYS_CDC_GetParam(VDRVCDCSel_t cdc_sel, VDRVCDCInfo_t info)
{
	...
	case VSYS_CDCState:
		...
		{
		case VSYS_CDC_CH0IN:
			val = VAIF_GetADCEnStatus();
			break;
		case VSYS_CDC_CH0OUT:
			val = VAIF_GetDACEnStatus();
			break;
		default:
			break;
		}
...
VRET_VALUE VSYS_CDC_SetParam(VDRVCDCSel_t cdc_sel, VDRVCDCInfo_t info, UINT32 val)
{
	...
	#endif
		break;
	case VSYS_CDCSyncSet:
		g_cdc_info[cdc_sel].sync_en = (BOOL)val;
		break;
	case VSYS_CDCSyncEn:
	{
		UINT8 Chn = 0;
		if(g_cdc_info[VSYS_CDC_CH0IN].sync_en && g_cdc_info[VSYS_CDC_CH0IN].len)
		{
			if (g_cdc_info[VSYS_CDC_CH0IN].channel == VSYS_CDC_LEFT_CHANNEL_ENABLE)
				Chn |= AIF_ADC_L;
			else if (g_cdc_info[VSYS_CDC_CH0IN].channel == VSYS_CDC_RIGHT_CHANNEL_ENABLE)
				Chn |= AIF_ADC_R;
			else
				Chn |= (AIF_ADC_L|AIF_ADC_R);
		}
		if(g_cdc_info[VSYS_CDC_CH0OUT].sync_en && g_cdc_info[VSYS_CDC_CH0OUT].len)
		{
			if (g_cdc_info[VSYS_CDC_CH0OUT].channel == VSYS_CDC_LEFT_CHANNEL_ENABLE)
				Chn |= AIF_DAC_L;
			else if (g_cdc_info[VSYS_CDC_CH0OUT].channel == VSYS_CDC_RIGHT_CHANNEL_ENABLE)
				Chn |= AIF_DAC_R;
			else
				Chn |= (AIF_DAC_L|AIF_DAC_R);
		}
		VAIF_EnableADCDAC(Chn);
		break;
	}
	default:
		return VRET_ERROR;
