...
static double vsine_wave_generate(double index, unsigned int freq, unsigned int sr, unsigned char dwidth, 
	unsigned char chan, unsigned char divisor, unsigned char *buff, unsigned short b_len)
{
	unsigned short sample_index = 0;
	double sample = (double)360*freq/sr;
	unsigned short period_len = vsine_wave_generate_len(freq, sr, dwidth, chan);
	unsigned short len = b_len >= period_len?period_len:b_len;
	...
	if (len < b_len)
	{
		...
		sample_index += (b_len - len)*sample_index/period_len;
	}
